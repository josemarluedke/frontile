...
test('component API sections require generated signatures', () => {
  const findings = lintDoc(
    join(fixtures, 'packages/frontile/src/components/example.md')
  );
  assert.ok(
    findings.some(({ message }) => message.includes('has no `<Signature />`'))
  );
});

test('modifier API sections allow hand-written typed tables', () => {
  const findings = lintDoc(
    join(fixtures, 'packages/frontile/src/modifiers/example.md')
  );
  assert.ok(
    !findings.some(({ message }) => message.includes('has no `<Signature />`'))
  );
