...
*also* decides the ARIA role or, for FormFeedback, whether the message is announced
...
service, `this.notifications.add('Saved', { status: 'success' })` — not on the component tag.
...
`soft` and `tonal` are also live, deprecated values, and they warn under the *same*
...
**`@appearance="soft"` changes meaning, not just name.** Old `soft` was a tint *with* a border;
new `@variant="soft"` is a tint *without* one. If you used `@appearance="soft"`, the value you
...
## Cross-level mixing gotcha (Listbox and its items)
