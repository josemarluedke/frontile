...
`@frontile/forms-legacy` are likewise deprecated — migrate to `frontile`'s own form components.
...
### `eq` is not available from `@ember/helper` — real trap

Ember's helper module does not export `eq`, `and`, `or`, `not`, etc. (those come from
`ember-truth-helpers`, which is not a Frontile dependency and may not exist in the consuming
app). Do not write `{{eq @foo "bar"}}` and assume it resolves — it will fail to compile/resolve.

Frontile's own source never reaches for an `eq` helper. Two verified patterns it uses instead:
...
1. **Push the comparison into a getter on the backing class**, then branch on the getter in the
   ...
   template uses `{{this.isSelected}}` / `data-selected="{{this.isSelected}}"` directly — no
...
If your app already depends on `ember-truth-helpers`, importing `eq` from there works fine —
Frontile just doesn't ship or require it.
...
- **Multi-slot components** (several themed parts, e.g. `Modal`) take `@classes` as a
  `SlotsToClasses<TheirSlots>` object — one key per slot — merged per-slot with the theme:
