...
module('Unit | Forms | Utils | nested-data', function (hooks) {
  ...
  module('getPath', function () {
    ...
    test('reads a dotted path through nested objects', function (assert) {
      ...
      assert.deepEqual(getPath(data, 'user.profile'), {
        email: 'a@example.com'
      });
    });
    ...
    test('refuses unsafe segments instead of reaching the prototype', function (assert) {
      ...
      assert.strictEqual(
        getPath(data, 'user.constructor.prototype'),
        undefined
      );
    });

    test('refuses an own __proto__ key, as JSON.parse produces', function (assert) {
      const data = JSON.parse('{"__proto__": {"polluted": true}}') as object;

      assert.strictEqual(getPath(data, '__proto__.polluted'), undefined);
    });

    test('reads @tracked fields and getters of class instances', function (assert) {
      class Profile {
        @tracked email = 'a@example.com';
      ...
      assert.strictEqual(getPath(data, 'profile.email'), 'a@example.com');
      assert.strictEqual(getPath(data, 'profile.domain'), 'example.com');
    });
  });
