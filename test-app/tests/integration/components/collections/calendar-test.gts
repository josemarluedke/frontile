...
module(
  ...
  function (hooks) {
    ...
    test('arrow keys focused on a header nav button do not hijack the day grid', async function (assert) {
      // Seeded rather than relying on the roving day defaulting to today --
      // this asserts on a specific `data-key`, so it would start failing the
      // day the clock rolls past it.
        ...
        <template>
          <Calendar @defaultMonth={{sep2026}} @locale="en-US">
            <:footer>
              ...
              {{! A fixed width, so this fixture stays wider than the grid no
                  matter what `--calendar-cell-size` is set to. Inline because
                  a utility class would have to be generated by Tailwind from
                  a test file. }}
