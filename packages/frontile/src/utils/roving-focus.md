...
```gts
<div role="radiogroup">
  <button aria-checked="true" {{roving.setupItem}}>...</button>
</div>
```

**`setupItem` takes no arguments.** Both facts it needs are read from the
element itself, live, at the moment a decision is made:

| State | How the element declares it |
| ----- | --------------------------- |
| selected | `data-selected="true"`, `aria-checked="true"`, or `aria-selected="true"` |
| disabled | `:disabled`, or `aria-disabled="true"` |

An element already has to declare both for the browser and for assistive
technology, so asking a consumer to declare them a second time only creates a
way for the two to disagree.

Covering several spellings of each is deliberate rather than defensive. A
radiogroup marks selection with `aria-checked`, a tab list with
`aria-selected`, and Frontile's own components publish `data-selected`.
Likewise `:disabled` is the form-control path, while `aria-disabled` is the
only option for an element that cannot be natively disabled at all — an `<a>`
or a `LinkTo`, which is exactly what a navigation tab list is built from.
...
or changes state. Because both facts are read from the element rather than
