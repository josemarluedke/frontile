...
const table = tv({
  slots: {
    ...
    wrapper: [
      ...
      'overflow-auto',
      'rounded-default',
      // Every sticky part of the table paints over the rows and columns that
      // scroll beneath it, so it needs a fill that is opaque *and* identical to
      // the table's own surface. Both come from this one variable — retarget it
      // (`[--table-surface:var(--color-surface-app)]`) to sit the table on a
      // different background without the sticky cells drifting out of step.
      '[--table-surface:var(--color-surface-card-solid)]',
    ...
    tfoot: ['relative', '[&>tr]:bg-[var(--table-surface)]'],
    tr: [
  ...
  compoundVariants: [
    ...
    // Sticky footer
    {
      ...
      class: {
        tfoot: ['sticky', 'bottom-0', 'z-2', 'bg-[var(--table-surface)]']
      }
    },
    ...
    // Sticky rows - base layer
    {
      ...
      stickyPosition: 'top',
      class: {
        tr: ['sticky', 'top-0', 'z-1', 'bg-[var(--table-surface)]']
      }
    },
    {
      isSticky: true,
      stickyPosition: 'bottom',
      class: {
        tr: ['sticky', 'bottom-0', 'z-1', 'bg-[var(--table-surface)]']
      }
    },
