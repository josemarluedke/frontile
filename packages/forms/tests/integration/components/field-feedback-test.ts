...
module('Integration | Component | FieldFeedback', function(hooks) {
  setupRenderingTest(hooks);

  const template = hbs`<FieldFeedback @id="feedback-id" @errors={{this.errors}} />`;
  ...
  test('it renders an array of errors', async function(assert) {
    ...
    assert.dom('[data-test-id="field-feedback"]').containsText('Some error');
    assert.dom('[data-test-id="field-feedback"]').hasClass('is-error');
  });
  ...
  test('it renders when passing an string', async function(assert) {
    ...
    await render(template);
    assert.dom('[data-test-id="field-feedback"]').containsText('Another error');
    assert.dom('[data-test-id="field-feedback"]').hasClass('is-error');
  });
  ...
  test('it does not break if passed an undefined arg', async function(assert) {
    ...
    await render(template);
    assert.dom('[data-test-id="field-feedback"]').exists();
    assert.dom('[data-test-id="field-feedback"]').doesNotHaveClass('is-error');
    assert
      .dom('[data-test-id="field-feedback"]')
  ...
  test('it renders passed in block', async function(assert) {
    await render(hbs`<FieldFeedback>Block content</FieldFeedback>`);
    assert.dom('[data-test-id="field-feedback"]').doesNotHaveClass('is-error');
    assert.dom('[data-test-id="field-feedback"]').containsText('Block content');
  });

  test('it adds errors class if @isError is passed in', async function(assert) {
    await render(
      hbs`<FieldFeedback @isError={{true}}>Block content</FieldFeedback>`
    );
    assert.dom('[data-test-id="field-feedback"]').hasClass('is-error');
    assert.dom('[data-test-id="field-feedback"]').containsText('Block content');
  });
