...
export default class ThemeSeam extends Component<Signature> {
  ...
  <template>
    ...
    <div
      class="theme-seam relative isolate overflow-hidden rounded-xl touch-pan-y select-none [--seam-handle-size:2.75rem]"
    ...
    >
      ...
      {{! Ambient theme }}
      <div inert>
        <span class="theme-seam__label field-label" aria-hidden="true">Your
          theme</span>
        {{yield to="specimen"}}
      </div>
      ...
      <div class="theme-seam__inverse theme-inverse" aria-hidden="true" inert>
        <span
          class="theme-seam__label field-label"
          aria-hidden="true"
        >Inverted</span>
        {{yield to="specimen"}}
      </div>
